...
class Transfer extends React.Component {
  ...
  renderPinForm() {
    if (!this.state.verifiedUser)
      return (
        <div><label><input className="transfer-form" type="number" placeholder=" Your Pin " value={this.state.user}
  ...
  handleChangeUser(e) {
    ...
    let userByName = this.getKeyByValue(pins, e.target.value);
    if (userByName)
  ...
  displayCuppers() {
    if (!this.state.transferee) {
      return this.cuppers.map((i) => {
        return <button key={i} id={i} className="btn btn-default transfer-to"
                       onClick={this.handleChangeTransferee}>{i}</button>
      })
    } else {
      return (<div>
        <p>Transfer to: {this.state.transferee} {this.showTransferFrom()}</p><br />
        {this.renderPinForm()}
        <label><input className="transfer-form" type="number" placeholder=" Chips " value={this.state.amount}
                      onChange={this.handleChangeAmount} /></label>
...
Transfer.propTypes = {
  chipCounts: PropTypes.array.isRequired,
